        ...
        while (ReceiveRequestsQueue.Count > 0)
        {
            ...
            switch (r)
            {
                                ...
                                if (password == request.password)
                                {
                                    ...
                                    if (suc)
                                    {
                                        ...
                                        if (BuildStoryDatabase.Instance.PlayerStoryStates.ContainsKey(username))
                                        {
                                            ...
                                            ClientBuildInfosRequest request1 = new ClientBuildInfosRequest(BuildStoryDatabase.Instance.GetPlayerBuilds(username), superAccount ? GamePlaySettings.ServerGamePlaySettings : GamePlaySettings.OnlineGamePlaySettings, true, story);
                                            SendMessage(request1);
                                        }
                                        else
                                        {
                                            ClientBuildInfosRequest request1 = new ClientBuildInfosRequest(BuildStoryDatabase.Instance.GetPlayerBuilds(username), superAccount ? GamePlaySettings.ServerGamePlaySettings : GamePlaySettings.OnlineGamePlaySettings, false);
                                            SendMessage(request1);
                                        }
                                    }
                                    else
                ...
                case StartNewStoryRequest _:
                {
                    ...
                    BuildStoryDatabase.Instance.RemovePlayerStory(UserName, this);
                ...
                case MatchStandAloneRequest request:
                    ...
                    if (ClientState == ClientStates.Login)
                    {
                        ...
                        {
                            Server.SV.SGMM.OnClientMatchStandAloneGames(this, request.LevelID);
                        }
                    }
