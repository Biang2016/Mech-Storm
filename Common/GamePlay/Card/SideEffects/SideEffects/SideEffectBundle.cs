    ...
    public string GetSideEffectsDesc()
    {
        ...
        foreach (KeyValuePair<TriggerTime, Dictionary<TriggerRange, List<SideEffectExecute>>> kv in SideEffectExecutes_Dict)
        {
            foreach (KeyValuePair<TriggerRange, List<SideEffectExecute>> SEEs in kv.Value)
            {
                ...
                {
                    if (SEEs.Key == TriggerRange.Self && (kv.Key == TriggerTime.OnHeroSummon || kv.Key == TriggerTime.OnRetinueSummon || kv.Key == TriggerTime.OnSoldierSummon)) res += BaseInfo.AddImportantColorToText(LanguageManager_Common.GetText("TriggerTime_Battlecry") + ": ");
                    else if (SEEs.Key == TriggerRange.Self && (kv.Key == TriggerTime.OnHeroDie || kv.Key == TriggerTime.OnRetinueDie || kv.Key == TriggerTime.OnSoldierDie)) res += BaseInfo.AddImportantColorToText(LanguageManager_Common.GetText("TriggerTime_Die") + ": ");
                    else if (SEEs.Key == TriggerRange.Self && (kv.Key == TriggerTime.OnEquipDie)) res += BaseInfo.AddImportantColorToText(LanguageManager_Common.GetText("TriggerTime_Die") + ": ");
                    else if (SEEs.Key == TriggerRange.Self && (kv.Key == TriggerTime.OnRetinueAttack)) res += LanguageManager_Common.GetText("TriggerTime_WhenAttack");
                    else if (SEEs.Key == TriggerRange.Self && (kv.Key == TriggerTime.OnRetinueKill)) res += LanguageManager_Common.GetText("TriggerTime_WhenKill");
                    ...
                    foreach (SideEffectExecute see in SEEs.Value)
                    {
                        res += see.SideEffectBase.GenerateDesc();
                    }
                }
            }
